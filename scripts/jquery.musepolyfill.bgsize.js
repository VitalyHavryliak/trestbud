(function (d) {
    ...
    if (!Muse.Browser.Features.checkCSSFeature("background-size")) {
        var c = function (a) {
                ...
                if (a === document.body) c = d("html"), a = c.get(0), g = b(c), h.src = g, c.css("background-attachment") == "fixed" ? (i.style.position =
                    "fixed", n = !1) : i.style.position = "absolute";
                else if (c.is("#page")) c.css("marginLeft").toLowerCase() == "auto" && (m = !0), i.style.top = c.offset().top + parseInt(c.css("borderTopWidth")) + "px", i.style.bottom = parseInt(c.parent().css("paddingBottom")) + parseInt(c.css("borderBottomWidth")) + "px", i.style.left = c.offset().left + parseInt(c.css("borderLeftWidth")) + "px", i.style.right = c.offset().left + parseInt(c.css("borderRightWidth")) + "px", i.style.zIndex = 0;
                ...
                this.reloadImage =
                    function () {
                        ...
                        if (c.data("hasBackgroundPositionScrollEffect") != !0)
                            for (var g = 0, k = d.length; g < k; g++) switch (d[g]) {
                                case "center":
                                case "50%":
                                    g == 0 ?
                                        (h.style.right = "", h.style.left = "50%", h.style.marginLeft = "-" + Math.ceil(h.offsetWidth / 2) + "px") : (h.style.bottom = "", h.style.top = "50%", h.style.marginTop = "-" + Math.ceil(h.offsetHeight / 2) + "px");
                                    break;
                                case "left":
                                    h.style.right = "";
                                    h.style.left = "0px";
                                ...
                                case "bottom":
                                    ...
                                    break;
                                default:
                                    g == 0 ? (h.style.left = d[g], h.style.marginLeft = "-" + Math.ceil(h.offsetWidth / 2) + "px") : (h.style.top = d[g], h.style.marginTop = "-" + Math.ceil(h.offsetHeight / 2) + "px")
                            }
                    };
                ...
                this.update = function () {
                    if (k) {
                        ...
                        if (b != "none" && (d == "cover" || d == "contain")) {
                            if (l.reloadImage(), i.style.display = "block", i.style.width = "0px", i.style.height = "0px", l.resizeImage(d == "cover"), m) i.style.left = c.offset().left + parseInt(c.css("borderLeftWidth")) + "px", i.style.right = c.offset().left + parseInt(c.css("borderRightWidth")) + "px"
                        } else i.style.display = "none"
                    }
                };
                if (h.complete || g == "none") k = !0;
                else d(h).one("load", function () {
                    k = !0;
                    l.update()
                });
                this.update()
        ...
        if (-1 != a) {
            ...
            for (var a = document.getElementsByTagName("meta"), b = 0, c = a.length; b < c; b++) {
                ...
                if ("generator" == d.getAttribute("name")) {
                    "2017.0.2.363" != d.getAttribute("content") && Muse.assets.outOfDate.push("jquery.musepolyfill.bgsize.js");
                    break
                }
            }
        }
